...
function useAnimatedBus(isRunning) {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      lastRef.current = now
      progressRef.current += delta * 0.012
      if (progressRef.current >= 0.95) { progressRef.current = 0.1; lastRef.current = null }
...
export default function LiveNavigationPage() {
  ...
  const latestSimPosRef = useRef([17.7048, 78.4816])
  ...
  useEffect(() => {
    latestSimPosRef.current = simulatedBusPosition
    latestSpeedRef.current = speed
  }, [simulatedBusPosition, speed])
  ...
  useEffect(() => {
    ...
    const fetchLiveLocation = async () => {
      try {
        ...
        if (mounted) setLiveLocation(res.data)
      } catch {
  ...
  useEffect(() => {
    ...
    const pushLiveLocation = async () => {
      const latestPos = latestSimPosRef.current
      let payload = {
        routeId: selectedRouteId,
        busNumber: route.busNumber,
        latitude: latestPos[0],
        longitude: latestPos[1],
          ...
          navigator.geolocation.getCurrentPosition(
            (position) => {
              payload = {
                ...
                speed: Number.isFinite(position.coords.speed) ? Math.max(0, position.coords.speed * 3.6) : latestSpeedRef.current,
      ...
      try {
        ...
        // Ignore temporary network or permission failures and keep fallback simulation.
      }
    }

    pushLiveLocation()
    const interval = setInterval(pushLiveLocation, 8000)
    return () => {
      mounted = false
      clearInterval(interval)
    }
  ...
  const nextStop = route.stops[Math.min(stopIdx + 1, totalStops - 1)] || 'HITAM Campus'
  const followStop = route.stops[Math.min(stopIdx + 2, totalStops - 1)] || 'HITAM Campus'
  ...
  const etaTimeA = new Date(Date.now() + etaMins * 60000).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })
  const etaTimeB = new Date(Date.now() + (etaMins + 8) * 60000).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{route.busNumber} · {route.name}</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="relative" style={{ height: 300 }}>
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-[#0F172A]/90 text-white px-4 py-2.5 rounded-xl flex items-center gap-3 shadow-xl whitespace-nowrap">
            ...
            <div>
              <p className="text-sm font-bold">{distToNext} km to {nextStop}</p>
          ...
          <div className="absolute bottom-4 right-4 flex flex-col gap-2">
            ...
            <button onClick={() => mapRef.current?.flyTo(busPosition, 13)} className="w-8 h-8 bg-[#40A047] rounded-lg shadow flex items-center justify-center"><Navigation size={13} className="text-white" /></button>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-sm font-bold text-gray-900">Next Stop: {nextStop}</p>
            ...
            {route.stops.slice(0, totalStops).map((stop, idx) => {
              ...
              return (
                <div key={idx} className={`flex-1 py-1 px-1 rounded-lg text-center ${isCur ? 'bg-[#40A047]' : passed ? 'bg-green-100' : 'bg-gray-100'}`}>
                  <p className="font-semibold truncate" style={{ fontSize: 8, color: isCur ? 'white' : passed ? '#166534' : '#6b7280' }}>{stop}</p>
                </div>
              )
            })}
          </div>
